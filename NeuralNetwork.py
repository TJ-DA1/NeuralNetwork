...
class NeuralNetwork:
    ...
    def backpropogate(self, data, expected, batches):
        testset = np.zeros((len(self.activations[-1]), batches))
        testset[expected, np.arange(batches)] = [1]
        self.calculatelayers(data)

        activationerror = (self.activations[-1] - testset) * self.activations[-1] * (1 - self.activations[-1])
        errorslist = [None] * (len(self.structure) - 1)
        errorslist[-1] = activationerror

        for i in range(len(errorslist) - 2, -1, -1):
            errorslist[i] = np.multiply((self.weights[i + 1].transpose() @ errorslist[i + 1]), derivativeleakyrelu(self.unactivated[i + 1]))
